<div>
    ...
    <div class="max-w-7xl w-full mx-auto mb-28">
        ...
        <!-- Einstellungen -->
        <div
            class="mt-4 py-4 border-2 text-sky-800 dark:text-gray-200 border-sky-800 dark:border-gray-700 bg-sky-50 dark:bg-slate-800 rounded-md shadow">
            <div x-data="{ open: false }">
                <div class="flex justify-between ">
                    <button x-on:click="open = ! open"
                        class="flex items-end justify-items-end w-full font-bold text-2xl px-3 py-1">
                        <span x-show="!open" aria-hidden="true" class="mr-2 mb-1 text-xl"><i
                        ...
                        <div class="flex items-end content-end">
                            ...
                            <div x-show="!open"
                                class ="flex-1 mb-1-2 text-gray-500 dark:text-gray-200 text-left text-sm line-clamp-1 italic font-extralight">
                                Kosteneingabe, Bankverbindung, Heizstromberechnung etc. </div>
                        </div>
                    </button>
                </div>
                <div x-show="open">
                    <div class="mx-1">
                        <livewire:user.realestate.abrechnung.einstellungen :baseobject='$realestate'
                            :wire:key="'modal-realestate-abrechnung-settings-'.$realestate->id" />
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="mt-2 mx-1 kostenliste rounded-xl">
            ...
            @forelse ($rows as $cost)
                <div class="columnheader rounded-xl">
                    ...
                    <h2>
                        ...
                        <div class="sm:text-sm md:text-sm lg:text-lg rounded-xl">
                            @if ($editable || $cost->costtype_id == 'BRK')
                                <div
                                    class="flex flex-row items-center justify-start border-b border-gray-400 font-semibold">
                                    ...
                                    <div class="basis-2/3 py-1 mr-1">
                                        <div class="flex px-2 justify-around gap-2 text-center">
                                            <div class=" {{ $editable ? 'basis-1/6' : 'basis-1/5' }} ">
                                                <span
                                                    class="{{ $this->realestate->eingabeCostDatum ? 'block' : 'hidden' }}">Datum</span>
                                            </div>
                                            <div class="{{ $editable ? 'basis-1/6' : 'basis-1/5' }}  px-2 items-center">
                                                <span
                                                    class="{{ $this->hasConsumptionByType('BRK', $this->realestate) ? 'block' : 'hidden' }} ">Verbrauch</span>
                                            </div>
                                            <div class="{{ $editable ? 'basis-1/6' : 'basis-1/5' }}">
                                                @if ($cost->co2Tax)
                        ...
                        <div class="flex justify-between m-0">
                            <div class="flex items-end content-end pl-3">
                                <div class="text-xl mb-1 font-semibold tracking-wider pr-1">
                                    {{ $cost->costtype->caption . '  (' . number_format($this->getCostByType('BRK', $this->realestate)->pluck('gros')->sum(), 2, ',', '.') . ' €)' }}
                                </div>
                            </div>
                            <button wire:click="addCostModal({{ $cost }})" tabindex="-1">
                    ...
                    <div>
                        ...
                        @forelse ($this->getCostByType($cost->costtype_id, $this->realestate) as $singleCost)
                            ...
                            <div key="{{ now() }}"
                                class="{{ $this->hasManyBrennstoffkosten($this->realestate) && $singleCost->costtype_id == 'BRK' ? 'border-4 border-sky-700 rounded-md m-2 py-2' : '' }}">
                                <!-- Anfangsbestand -->
                                @if ($singleCost->fueltype_id != null && $singleCost->fueltype->hasTank)
                                    <div
                                        class="my-2 m-1 flex flex-row items-center justify-start font-normal text-lg h-10 border-b border-gray-400 text-center ">
                                        <div class="basis-1/3  py-1">
                                            <div class="flex justify-start px-2 items-center ">
                                                <div class="text-lg text-center">
                                                    <span
                                                        class="">{{ 'Anfangsbestand ' . $singleCost->caption . ' [' . $singleCost->fueltype->einheit->shortname . ']' }}</span>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="basis-2/3 py-1 ">
                                            <div class="flex gap-2 ">
                                                <div class="{{ $editable ? 'basis-1/6' : 'basis-1/5' }}">
                                                    <div
                                                        class="{{ $this->realestate->eingabeCostDatum ? 'block' : 'hidden' }} text-lg text-center ">
                                                    </div>
                                                </div>
                                                <div class="{{ $editable ? 'basis-1/6' : 'basis-1/5' }}">
                                                    <div
                                                        class="{{ $this->hasConsumptionByType('BRK', $this->realestate) ? 'block' : 'hidden' }} text-center">
                                ...
                                <!-- Kosten-Eingabe -->
                                <div
                                    class="flex flex-row {{ $singleCost->costAmounts->count() > 0 && $editable ? 'border-b-2' : 'border-b-0' }} items-center justify-start font-normal text-lg ">
                                    @if (
                                        !$realestate->abrechnungssetting->brennstofflisteDone ||
                                            ($singleCost->fueltype_id != null && $singleCost->fueltype->hasTank))
                                        <div class="basis-1/3 py-1 ">
                                            <button wire:click="editCostModal({{ $singleCost }})" tabindex="-1"
                                                class="flex py-0.5 w-full rounded-sm hover:bg-sky-300 dark:hover:bg-slate-900 px-2 items-center justify-start ">
                                                <div class="text-lg">
                                                    @if (
                                                        $singleCost->fueltype_id != null &&
                                                            $singleCost->fueltype->hasTank &&
                                                            !$realestate->abrechnungssetting->brennstofflisteDone)
                                    ...
                                    @if (!$realestate->abrechnungssetting->brennstofflisteDone)
                                        <div class="basis-2/3 py-1">
                                            <livewire:user.cost-amount.detail-input-br :cost='$singleCost'
                                                :wire:key="'list-cost-costamountinput-'.$singleCost->id"
                                                key="{{ now() }}" />
                                        </div>
                                    @else
                                        <!-- Kosten-Ansicht -->
                                ...
                                <div
                                    class=" {{ $singleCost->costAmounts->count() > 0 ? 'block  bg-sky-200 dark:bg-slate-900' : 'invisible' }} items-center justify-start font-normal text-lg ">
                                    ...
                                    @foreach ($singleCost->costAmounts as $singleCostAmount)
                                        @if (
                                            $singleCostAmount->abrechnungssetting_id == $cost->realestate->abrechnungssetting_id &&
                                                !$singleCostAmount->startvalue &&
                                                !$singleCostAmount->endvalue)
                                            <div class="flex items-center font-light">
                                                    ...
                                                    <div class="text-sm pl-2">
                                                        ...
                                                        @else
                                                            @if ($singleCostAmount->created_at)
                                                                <span
                                                                    class="">{{ 'Eintrag vom: ' . $singleCostAmount->created_at->format('d.m.Y') }}</span>
                                                            @else
                                                                <span
                                                                    class="">{{ $singleCostAmount->description }}</span>
                                                            @endif
                                                        @endif
                                                    </div>
                                                </div>
                                                <div class="basis-2/3 py-1 ">
                                                    <div
                                                        class="flex items-center px-2 py-1 justify-around gap-2 text-center md:text-lg">
                                                        @if ($singleCost->fueltype_id != null && $singleCost->fueltype->hasTank)
                                                            <div id="user-costamount-listitem-datum{{ $singleCostAmount->id }}"
                                                                ...
                                                                <span
                                                                    class="">{{ $singleCostAmount->datum }}</span>
                                                            </div>
                                                        @else
                                                            <div id="user-costamount-listitem-datum{{ $singleCostAmount->id }}"
                                                                style="-moz-appearance: textfield; margin: 0;"
                                                        ...
                                                        <div id="user-costamount-listitem-consumption{{ $singleCostAmount->id }}"
                                                            ...
                                                            <span
                                                                class="">{{ $singleCostAmount->consumption_editing }}</span>
                                                        </div>
                                                        <div id="user-costamount-listitem-co2TaxValue{{ $singleCostAmount->id }}"
                                                            ...
                                                            @if ($singleCost->co2Tax)
                                                                <span
                                                                    class="">{{ $singleCostAmount->coconsupmtion }}</span>
                                                            @endif
                                                        </div>
                                                        <div id="user-costamount-listitem-haushaltsnah{{ $singleCostAmount->id }}"
                                                            ...
                                                            @if ($singleCost->co2Tax)
                                                                @if ($this->realestate->eingabeCostNetto)
                                                                    <span
                                                                        class="">{{ $singleCostAmount->conetto }}</span>
                                                                @else
                                                                    <span
                                                                        class="">{{ $singleCostAmount->cobrutto }}</span>
                                                                @endif
                                                            @endif
                                                        </div>
                                                        <div id="user-costamount-listitem-betrag{{ $singleCostAmount->id }}"
                                                            ...
                                                            @if ($this->realestate->eingabeCostNetto)
                                                                <span
                                                                    class="">{{ $singleCostAmount->netto }}</span>
                                                            @else
                                                                <span
                                                                    class="">{{ $singleCostAmount->brutto }}</span>
                                                            @endif
                                                        </div>

                                                        <div class="{{ $editable ? 'basis-1/6' : 'hidden' }}">
                                                            <div class="flex">
                                                                <div wire:click="editCostAmountModal({{ $singleCostAmount }})"
                                                                ...
                                                                <div wire:click="questionDeleteCostAmount({{ $singleCostAmount }})"
                                                                    ...
                                                                    <x-icon.fonts.trash
                                                                        class="text-blue-800 "></x-icon.fonts.trash>
                                                                </div>
                                                            </div>
                                ...
                                <!-- Summenfeld -->
                                <div
                                    class="font-semibold {{ $cost->costAmounts->count() > 0 ? 'border-y block dark:bg-slate-900' : 'hidden' }}  items-center justify-start font-normal md:text-lg">
                                    <div
                                        class="{{ $singleCost->costtype_id == 'BRK' || $singleCost->costAmounts->count() > 1 ? 'flex px-2 items-center dark:bg-slate-800' : 'hidden' }}">
                                        ...
                                        <div class="basis-2/3 text-center">
                                            <div class="flex justify-start gap-1 items-center">
                                                ...
                                                <div class="{{ $editable ? 'basis-1/6' : 'basis-1/5' }} ">
                                                    <span
                                                        class="py-1 {{ $singleCost->consumption ? 'block' : 'hidden' }} ">{{ $singleCost->consumptionsum }}</span>
                                                </div>
                                                <div class="{{ $editable ? 'basis-1/6' : 'basis-1/5' }}"></div>
                                                <div class="{{ $editable ? 'basis-1/6' : 'basis-1/5' }}"></div>
                                                <div class="{{ $editable ? 'basis-1/6' : 'basis-1/5' }}">
                                ...
                                @if ($singleCost->fueltype_id != null && $singleCost->fueltype->hasTank)
                                    <div
                                        class="border-b border-gray-400 m-2 gap-2 flex flex-row items-center justify-start font-normal text-lg h-10 ">
                                        <div
                                            class="basis-1/3 py-1 px-2 flex justify-start items-center text-lg text-center">
                                            <span
                                                class="">{{ 'Endbestand ' . $singleCost->caption . ' [' . $singleCost->fueltype->einheit->shortname . ']' }}</span>
                                        </div>
                                        <div class="basis-2/3 py-1 ">
                                            <div class="flex justify-around gap-2 text-lg">
                                                ...
                                                <div class="flex justify-center basis-1/6 items-center">
                                                    <div
                                                        class="{{ $this->hasConsumptionByType('BRK', $this->realestate) ? 'block' : 'hidden' }} text-center flex items-center">
                                                        @if ($editable)
                                                            <div wire:click="editEndstand({{ $singleCost }})"
                                                                ...
                                                                <span class="md:text-md "><i
                                                                        class="text-left pr-1 fa-solid fa-pencil"></i></span>
                                                                @if ($singleCost->end_value_editing <= '0,0')
                                                                    <span class="md:text-md text-right">Eingabe</span>
                                                                @else
                                                                    <span
                                                                        class="md:text-md text-right">{{ $singleCost->end_value_editing }}</span>
                                                                @endif
                                                            </div>
                                                        @else
                                                            <div class="text-center">
                                                                {{ $singleCost->end_value_editing }}</div>
                                                        @endif
                                                    </div>
                                                </div>
                                                ...
                                                <div class="flex justify-around basis-1/6 items-center "></div>
                                                <div
                                                    class="flex {{ $editable ? 'block' : 'hidden' }} justify-around basis-1/6 px-2 items-center ">
                                                </div>
                                                <div class="flex justify-around basis-1/6 items-center "></div>
                                            </div>
                                        </div>
